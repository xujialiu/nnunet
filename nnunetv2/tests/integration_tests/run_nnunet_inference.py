...
def run_tests_and_exit_on_failure():
    ...
    # Run nnunet
    subprocess.call(
        f"nnUNetv2_predict -i nnunetv2/tests/github_actions_output -o nnunetv2/tests/github_actions_output -d 300 -tr nnUNetTrainer -c 3d_fullres -f 0 -device cpu",
        shell=True,
    )

    # Check if the nnunet segmentation is correct
    img_gt = nib.load(
        f"nnunetv2/tests/example_data/example_ct_sm_T300_output.nii.gz"
    ).get_fdata()
    img_pred = nib.load(
        f"nnunetv2/tests/github_actions_output/example_ct_sm.nii.gz"
    ).get_fdata()
