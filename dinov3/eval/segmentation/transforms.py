...
class FixedSideResize:
    ...
    def _resize(self, img, label, small_size):
        ...
        # For TTA with deformable attention, both dimensions must be multiples of 32
        # to ensure SPM conv outputs match reference_points from integer division
        if self.use_tta:
            new_height = int(np.ceil(new_height / 32)) * 32
            new_width = int(np.ceil(new_width / 32)) * 32
    ...
    def __call__(self, img, label):
        ...
        for tta_ratio in self.tta_img_size_ratio_range:
            ...
            # Round to multiple of 32 for all ratios to ensure SPM conv outputs
            # match the reference_points computed via integer division in deform_inputs()
