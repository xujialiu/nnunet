...
def build_segmentation_decoder(
    ...
    if decoder_type == "m2f":
        ...
        # Only freeze backbone if LoRA is NOT enabled
        if not lora_enabled:
            backbone_model.requires_grad_(False)
        else:
            # With LoRA: freeze base weights but keep LoRA params trainable
            # PEFT already handles this, but we need to ensure the wrapper doesn't break it
...
def log_trainable_parameters(model: torch.nn.Module, logger) -> None:
    """Log all trainable parameters with their names and shapes.
    ...
    for name, numel, shape in trainable_params:
        logger.info(f"  {name}: {shape} ({numel:,} params)")
    logger.info("-" * 60)
    logger.info(f"Total trainable: {total_trainable:,} ({100 * total_trainable / total_params:.2f}%)")
    logger.info(f"Total frozen: {total_frozen:,} ({100 * total_frozen / total_params:.2f}%)")
