...
def make_inference(
    ...
    if inference_mode == "slide":
        ...
        # Whole mode: use image as-is (already sized by transforms.eval.img_size)
        ...
        # Move to CPU for consistency with slide mode (which accumulates on CPU)
        pred = pred.cpu()
    ...
    for h_idx in range(h_grids):
        for w_idx in range(w_grids):
            ...
            if decoder_head_type == "m2f":
                ...
                mask_cls = F.softmax(mask_cls, dim=-1)[..., :-1]
                mask_pred = mask_pred.sigmoid()
                crop_pred = torch.einsum("bqc,bqhw->bchw", mask_cls.to(torch.float), mask_pred.to(torch.float))
