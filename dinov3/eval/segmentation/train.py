...
def train_segmentation(
    ...
    if config.lora.enabled:
        for k, v in state_dict.items():
            if "lora_" in k:
                save_dict[k] = v
        logger.info(f"Saving LoRA weights: {[k for k in save_dict.keys() if 'lora_' in k][:5]}...")

    torch.save(
        {
            "model": save_dict,
            "optimizer": optimizer.state_dict(),
            "lora_config": config.lora if config.lora.enabled else None,  # Save LoRA config for loading
        },
